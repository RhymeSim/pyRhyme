...
class InlineCombination():
        ...
        self.modes = {
            'q': {
                ...
                'actions': {
                    ...
                    'e': {
                        'desc': 'Execute a VisItAPI or VisIt command',
                        'ex': 'qe reset_view(), qe Expressions()',
            ...
            'h': {
                ...
                'actions': {
                    ...
                    'w': {
                        'desc':
                        'Print current woindow info',
                        'ex':
                        'iw, iwcycle',
                        'run':
                        lambda v, k: v.get_window_metadata(print_it=True,
                                                           key=k),
                    },
                    'v': {
                        'desc':
                        ...
                        'iv',
                        'run':
                        lambda v, _: v.get_window_metadata(print_it=True,
                                                           key='variables'),
                    },
                }
            },
            't': {
                ...
                'actions': {
                    't': {
                        'desc': 'Find snapshot with closes time',
                        'ex': 'tt16.5',
            ...
            'd': {
                ...
                'actions': {
                    ...
                    'p': {
                        ...
                        'ex': 'dp, dp10',
                        'run':
                        lambda v, l: self.play_cycles(v, l, reset_view=False),
                        'after':
                        lambda v, _: v.cycle(self.cycle, reset_view=False),
                    },
                    'f': {
                        'desc': 'Play and reset the view',
                        'ex': 'df, df10',
                        'run':
                        lambda v, l: self.play_cycles(v, l, reset_view=True),
                        'after':
                        lambda v, _: v.cycle(self.cycle, reset_view=True),
                    },
                    'm': {
                        'desc': 'Record a movie of snapshots',
                        'ex': 'dr, dr10',
                        'run':
                        lambda v, l: self.play_cycles(
                            v, l, reset_view=True, save_it=True),
                        'after':
                        lambda v, _: v.cycle(self.cycle, reset_view=True),
                    },
                }
    ...
    def try_run_a_command(self, visitapi, command_str):
        ...
        try:
            # VisItAPI Command
            print(
                eval("getattr(visitapi, '" + method_name + "')(" + args + ")"))
